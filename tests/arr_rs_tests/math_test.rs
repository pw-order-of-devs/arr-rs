...
case(vec![1, 2, 3, 4], vec![2, 2], 24),
    ...
    assert_eq!(expected, Array::new(elements, shape).unwrap().product())
}
...
case(vec![1, 2, 3, 4], vec![2, 2], 10),
    ...
    assert_eq!(expected, Array::new(elements, shape).unwrap().sum())
}
...
case(array!([[1, 2], [3, 4]]), array!([1, 3, 6, 10])),
    ...
    assert_eq!(expected.unwrap(), array.unwrap().cumsum())
}

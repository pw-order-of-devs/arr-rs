...
pub trait ArrayBroadcast<N: Numeric> where Self: Sized + Clone {
    ...
    ///     (2, 4), (2, 5), (2, 6),
    ///     (3, 4), (3, 5), (3, 6)
    /// ].into_iter().map(Tuple2::from_tuple).collect(), vec![3, 3]);
    ///
    /// let arr_1: Array<i32> = array!([[1], [2], [3]]);
    /// let arr_2: Array<i32> = array!([[4, 5, 6]]);
    ///
    /// let broadcast: Array<Tuple2<i32>> = arr_1.broadcast(&arr_2);
    ...
    /// use arr_rs::prelude::*;
    ///
    /// let expected: Array<i32> = Array::new(vec![1, 1, 1, 2, 2, 2, 3, 3, 3], vec![3, 3]);
    /// let arr_1: Array<i32> = array!([[1], [2], [3]]);
    ///
    /// let broadcast: Array<i32> = arr_1.broadcast_to(vec![3, 3]);
    /// assert_eq!(expected, broadcast);
    /// ```
    ...
    ///     Array::new(vec![1, 1, 1, 2, 2, 2, 3, 3, 3], vec![3, 3]),
    ///     Array::new(vec![4, 5, 6, 4, 5, 6, 4, 5, 6], vec![3, 3]),
    /// ];
    /// let arr_1: Array<i32> = array!([[1], [2], [3]]);
    /// let arr_2: Array<i32> = array!([4, 5, 6]);
    ///
    /// let broadcast: Vec<Array<i32>> = Array::broadcast_arrays(vec![arr_1 ,arr_2]);
    /// assert_eq!(expected, broadcast);
    /// ```

...
impl <T: ArrayElement> ArrayAxis<T> for Array<T> {
    ...
    fn apply_along_axis<S: ArrayElement, F>(&self, axis: usize, mut f: F) -> Result<Array<S>, ArrayError>
        ...
        let array = self.moveaxis(vec![axis.to_isize()], vec![self.ndim()?.to_isize()])?;
        let partial = array
            .ravel()
        ...
        if axis == 0 { partial.rollaxis((self.ndim()? - 1).to_isize(), None) }
        else { partial.moveaxis(vec![axis.to_isize()], vec![(self.ndim()? - 1).to_isize()]) }
    }
    ...
    fn transpose(&self, axes: Option<Vec<isize>>) -> Result<Self, ArrayError> {
        ...
        fn transpose_recursive<T: ArrayElement>(
            ...
            if current_dim < input_shape.len() - 1 {
                ...
                (0..input_shape[current_dim]).for_each(|i| {
                    ...
                    let output_indices = axes.as_ref().map_or_else(
                        || current_indices.iter().rev().copied().collect::<Vec<usize>>(),
                        |axes| axes.iter().map(|&ax| current_indices[ax]).collect::<Vec<usize>>());
        ...
        let new_shape: Vec<usize> = axes.clone().map_or_else(
            || self.shape.clone().into_iter().rev().collect(),
            |axes| axes.into_iter().map(|ax| self.shape[ax]).collect());
